...
echo mshfabL: fabricating $appid update script..



## RFEM
if [[ $appid = 'rfem' && $valid -eq 1 ]]; then

	mshfile=${scrdir}rfem_update.msh

	echo "/* autogenerated by mshfabL.sh `date` on $envid */" > $mshfile
...
## RPLA, RPLB, RPLU
if [[ $apptype = 'rpl' && $valid -eq 1 ]]; then

	mshfile=${scrdir}${appid}_update.msh
	echo making ${appid}_update.msh $mshfile
	echo "/* autogenerated by mshfabL.sh `date` on $envid */" > $mshfile
	echo 'login system manager on localhost;' >> $mshfile
	...
	echo 'logout; exit;' >> $mshfile
	echo mshfabL: done $appid

fi


## RPLU
if [[ $appid = 'rplu' && $valid -eq 1 ]]; then
